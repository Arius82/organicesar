...
-- Allow masters to update roles (for delegation)
...
CREATE POLICY "Authenticated can insert tasks" ON public.tasks FOR INSERT TO authenticated WITH CHECK (public.has_role(auth.uid(), 'master') OR auth.uid() = usuario_id);
CREATE POLICY "Authenticated can update tasks" ON public.tasks FOR UPDATE TO authenticated USING (public.has_role(auth.uid(), 'master') OR auth.uid() = usuario_id);
...
CREATE POLICY "Masters can delete pantry" ON public.pantry_items FOR DELETE TO authenticated USING (true);
...
CREATE POLICY "Masters can delete meals" ON public.meal_plans FOR DELETE TO authenticated USING (true);
